// tympan.browser.js · auto-generated browser-global build of tympan.js (no ES modules; assigns window.__ORBO_TYMPAN).
// Source of truth is tympan.js. Regenerate this file if tympan.js changes, don't hand-edit.
// Load order: AFTER mater.browser.js (the Tympan imports the Mater), before framing/astrodna/rulers.
(function boot(){
...
const { SIGNS, RULERS, DISPOSITORS, signIndexOf } = window.__ORBO_MATER;
...
const HOUSE_FRAMES = Object.freeze(SIGNS.map((_, asc) => Object.freeze(SIGNS.map((__, sg) => ((sg - asc + 12) % 12) + 1))));
...
const SIGN_FRAMES = Object.freeze(SIGNS.map((_, asc) => Object.freeze(SIGNS.map((__, i) => (asc + i) % 12))));
...
const RULES_HOUSES = Object.freeze(HOUSE_FRAMES.map((frame) => Object.freeze(Object.fromEntries(
...
// absence is null.
const MODERNS = Object.freeze(['Uranus', 'Neptune', 'Pluto']);
const MODERN_RULERS = Object.freeze({ 7: 'Pluto', 10: 'Uranus', 11: 'Neptune' });
const MODERN_SIGNS = Object.freeze(Object.keys(MODERN_RULERS).map(Number));
const CO_RULES_HOUSES = Object.freeze(HOUSE_FRAMES.map((frame) => Object.freeze(Object.fromEntries(
...
const FRAMES = Object.freeze(SIGNS.map((_, asc) => Object.freeze({
  ascSign: asc,
...
// ---------- the reads ----------
function frameOf(ascSignIdx) { return FRAMES[signAddr(ascSignIdx, 'frameOf')]; }
function houseFrame(ascSignIdx) { return HOUSE_FRAMES[signAddr(ascSignIdx, 'houseFrame')]; }
function houseOfSign(signIdx, ascSignIdx) { return HOUSE_FRAMES[signAddr(ascSignIdx, 'houseOfSign')][signAddr(signIdx, 'houseOfSign')]; }
...
function houseOfLon(bodyLon, ascendantLon) { return houseOfSign(signIndexOf(bodyLon), signIndexOf(ascendantLon)); }
function signOfHouse(house, ascSignIdx) { return SIGN_FRAMES[signAddr(ascSignIdx, 'signOfHouse')][houseOrd(house, 'signOfHouse') - 1]; }
function rulerOfHouse(house, ascSignIdx) { return RULERS[signOfHouse(house, ascSignIdx)]; }
function coRulerOfSign(signIdx) { return MODERN_RULERS[signAddr(signIdx, 'coRulerOfSign')] || null; }
function coRulerOfHouse(house, ascSignIdx) { return coRulerOfSign(signOfHouse(house, ascSignIdx)); }
...
function housesRuledBy(planet, ascSignIdx) { return RULES_HOUSES[signAddr(ascSignIdx, 'housesRuledBy')][governor(planet, 'housesRuledBy')]; }
function housesCoRuledBy(planet, ascSignIdx) { return CO_RULES_HOUSES[signAddr(ascSignIdx, 'housesCoRuledBy')][anyGovernor(planet, 'housesCoRuledBy')]; }
...
const FLIP_HOUSES = 6;
function flipHouse(house) { return ((houseOrd(house, 'flipHouse') - 1 + FLIP_HOUSES) % 12) + 1; }
...
window.__ORBO_TYMPAN = { HOUSE_FRAMES, SIGN_FRAMES, RULES_HOUSES, CO_RULES_HOUSES, MODERNS, MODERN_RULERS, FRAMES, FLIP_HOUSES, frameOf, houseFrame, houseOfSign, houseOfLon, signOfHouse, rulerOfHouse, coRulerOfSign, coRulerOfHouse, housesRuledBy, housesCoRuledBy, flipHouse };
})();
