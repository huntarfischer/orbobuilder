// rulers.browser.js — auto-generated browser-global build of rulers.js (no ES modules; assigns window.__ORBO_RULERS).
// Source of truth is rulers.js — regenerate this file if rulers.js changes, don't hand-edit.
// Load order: after mater.browser.js, its one dependency.
(function boot(){
...
const { SIGNS, DOMICILE, EXALTATION, DISPOSITORS, dignityOfSign, elementOf } = window.__ORBO_MATER;
...
function chartRulerOf(dna) { return lordOf(dna.nodes.Ascendant.longitude).ruler; }
...
const EGYPTIAN_BOUNDS = Object.freeze([
...
const CHALDEAN = Object.freeze(['Saturn', 'Jupiter', 'Mars', 'Sun', 'Venus', 'Mercury', 'Moon']);
const FACES = Object.freeze(Array.from({ length: 36 }, (_, i) => CHALDEAN[(CHALDEAN.indexOf('Mars') + i) % 7]));
...
const TRIPLICITY = Object.freeze({
...
function triplicityOf(signIdx, isDay) {
...
function triplicityOfLon(lonDeg, isDay) { return triplicityOf(Math.floor(norm(lonDeg) / 30), isDay); }
...
function ladderOf(planet, lonDeg, isDay) {
...
window.__ORBO_RULERS = { SIGNS, DOMICILE, EXALTATION, DISPOSITORS, lordOf, dignityOf, chartRulerOf,
  EGYPTIAN_BOUNDS, FACES, CHALDEAN, TRIPLICITY, boundOf, faceOf, triplicityOf, triplicityOfLon, ladderOf };
})();
