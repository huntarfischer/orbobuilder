// progressions.browser.js — auto-generated browser-global build of progressions.js (no ES modules; assigns window.__ORBO_PROGRESSIONS).
// Source of truth is progressions.js — regenerate this file if progressions.js changes, don't hand-edit.
// No dependencies: pure engine, no ephemeris access — load order does not matter.
(function(){
...
const NAIBOD_RATE = 0.98564733; // deg/year, mean solar motion in right ascension
...
function ageYears(natalJd, jd) { return (jd - natalJd) / TROPICAL_YEAR_DAYS; }
function progressedJd(natalJd, jd) { return natalJd + ageYears(natalJd, jd); }
function realJdFromAge(natalJd, age) { return natalJd + age * TROPICAL_YEAR_DAYS; }
...
function anglesFromRamc(ramcDeg, latDeg, epsDeg) {
...
function ramcFromMc(mcLonDeg, epsDeg) {
...
function progressedAngles(policy, ctx) {
...
function build(natal, doctrine, sample, opts = {}) {
...
function at(table, natal, jd, exactSample) {
...
function tableKey(natal, doctrine) {
  return natal.jd + '|' + ((doctrine && doctrine.progAngle) || 'naibod');
}

window.__ORBO_PROGRESSIONS = { ageYears, progressedJd, realJdFromAge, anglesFromRamc, ramcFromMc, progressedAngles, build, at, tableKey, TROPICAL_YEAR_DAYS, NAIBOD_RATE, BODIES };
})();
