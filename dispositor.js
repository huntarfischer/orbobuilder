...
import { RULERS, DISPOSITORS, EXALT_BY_SIGN } from './mater.js';
...
export function isDispositorCapable(name) { return DISPOSITORS.includes(name); }
...
export function bearerOf(signIdx) { return RULERS[signAddr(signIdx, 'bearerOf')]; }
...
export function receptions(signs) {
  const out = [];
...
// SIBLING EXPORTS, never folded inside walk(). One set's structure and two sets' relations are two
// different questions, exactly as receptions() is a sibling rather than a field of the walk — and
// `express` takes ONE map, which is why "A's Venus in the sign B's Mars rules" could not be produced
// at all before this.
//
// ONE STEP ACROSS, NEVER AN ALTERNATING WALK. A handoff records where A's occupant's bearer SITS in
// B's set, and stops. Continuing (B's bearer's own bearer back in A, and so on) is deliberately not
// built: the same refusal as compositing two composites — the answer would be an artifact of which
// set you started from rather than a fact about the two. `lands` is null when the bearer's own sign
// was never supplied by that set: leaving the frame is the honest answer here too.
//
// A SAME-NAME PAIR IS NOT A CROSS RELATION, and skipping it is the whole subtlety. A's Venus in
// Taurus is "received" by B's Venus only because Venus rules Taurus in every set there is, so the
// fact belongs to A alone (its own dignity) while reading as a relation between the two. Recorded
// nowhere. And THE CLASSICAL AT-HOME EXCLUSION IS STILL A BRANCH HERE: domicile cases exclude
// themselves (RULERS[sa] === b with a ≠ b already fails for a planet at home), but a planet in its
// own sign can still sit in the OTHER set's planet's exaltation — A's Mars home in Aries would read
// as received by B's Sun — and a host is not a guest, per the same rule receptions() carries.
export function crossReceptions(signsA, signsB) {
  const out = [];
...
export function crossHandoffs(signsA, signsB) {
...
export function crossDispose(signsA, signsB) {
