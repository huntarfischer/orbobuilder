...
import { norm360, wrap180, midpoint, refract, signIndex, phaseOf, mod180, RULERS, SIGNS, BODIES,
         floorTargets, contactTargets, synchronicTargets, synOrb } from './framing.js';
...
export const STEP_FOR = { Moon: 0.15, Sun: 0.5, Mercury: 0.35, Venus: 0.5, Mars: 0.6, Jupiter: 1.5,
  Saturn: 2, Uranus: 3, Neptune: 4, Pluto: 4, Node: 2, SNode: 2, Chiron: 3, Lilith: 0.6 };
export function stepFor(body, mode) {
...
export const CONFIRM = { floor: 0.05, contact: 0.05, synchronic: 0.1 };
...
export const CONFIRM_SPEED = 5e-4;
...
export function scanTargets(spec) {
  ...
  // through framing.refract, the app's ONE refraction door (Phase 6 · P1), so the scanner and the
  // instrument cannot drift apart on what a synchronic degree is.
...
export function decorate(root, ctx = {}) {
...
export function buildContact(natal, spec) {
...
export function buildSynchronic(natal, spec) {
...
export function retroPeriods(floorEvents) {
  const by = {}, out = [];
...
export function loomBuild(layer, natal, spec) {
