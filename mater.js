...
export const ELEMENTS = Object.freeze(['fire', 'earth', 'air', 'water']);
export const MODALITIES = Object.freeze(['cardinal', 'fixed', 'mutable']);

export const SIGN_INDEX = Object.freeze(Object.fromEntries(SIGNS.map((s, i) => [s, i])));
export const SIGN_ELEMENT = Object.freeze(SIGNS.map((_, i) => ELEMENTS[i % 4]));
export const SIGN_MODALITY = Object.freeze(SIGNS.map((_, i) => MODALITIES[i % 3]));
// name-keyed element, the shape astrodna.js reads.
export const SIGN_ELEMENTS = Object.freeze(Object.fromEntries(SIGNS.map((s, i) => [s, ELEMENTS[i % 4]])));
export const SIGN_TABLE = Object.freeze(SIGNS.map((name, i) => Object.freeze({
...
export const RULERS = Object.freeze(['Mars', 'Venus', 'Mercury', 'Moon', 'Sun', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Saturn', 'Jupiter']);
// name-keyed, the shape rulers.js and astrodna.js read.
export const DOMICILE = Object.freeze(Object.fromEntries(SIGNS.map((s, i) => [s, RULERS[i]])));
...
export const DISPOSITORS = Object.freeze(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn']);
// Which signs each planet rules, stamped rather than searched (Mercury and Venus, Mars, Jupiter and
// Saturn each hold two).
export const RULES_SIGNS = Object.freeze(Object.fromEntries(DISPOSITORS.map((p) => [p, Object.freeze(RULERS.reduce((o, r, i) => (r === p ? o.concat(i) : o), []))])));
...
export const EXALT = Object.freeze({ Sun: 0, Moon: 1, Mercury: 5, Venus: 11, Mars: 9, Jupiter: 3, Saturn: 6 });
export const EXALT_DEGREE = Object.freeze({ Sun: 19, Moon: 3, Mercury: 15, Venus: 27, Mars: 28, Jupiter: 15, Saturn: 21 });
// sign-keyed, the shape rulers.js reads.
export const EXALTATION = Object.freeze(Object.fromEntries(Object.keys(EXALT).map((p) => [SIGNS[EXALT[p]], Object.freeze({ planet: p, degree: EXALT_DEGREE[p] })])));
export const EXALT_BY_SIGN = Object.freeze(SIGNS.map((s) => (EXALTATION[s] ? EXALTATION[s].planet : null)));
...
export const DETRIMENT_BY_SIGN = Object.freeze(SIGNS.map((_, i) => RULERS[(i + 6) % 12]));
export const FALL_BY_SIGN = Object.freeze(SIGNS.map((_, i) => EXALT_BY_SIGN[(i + 6) % 12]));
export const DETRIMENT = Object.freeze(Object.fromEntries(DISPOSITORS.map((p) => [p, Object.freeze(RULES_SIGNS[p].map((i) => (i + 6) % 12))])));
export const FALL = Object.freeze(Object.fromEntries(Object.keys(EXALT).map((p) => [p, (EXALT[p] + 6) % 12])));

// ---------- the reads ----------
export const signIndexOf = (lon) => Math.floor(norm360(lon) / 30) % 12;
export const signNameOf = (lon) => SIGNS[signIndexOf(lon)];
export const glyphOf = (signIdx) => SIGN_GLYPHS[((signIdx % 12) + 12) % 12];
export const elementOf = (signIdx) => SIGN_ELEMENT[((signIdx % 12) + 12) % 12];
export const modalityOf = (signIdx) => SIGN_MODALITY[((signIdx % 12) + 12) % 12];
export const rulerOf = (signIdx) => RULERS[((signIdx % 12) + 12) % 12];
...
export function dignityOfSign(planet, signIdx) {
...
export const dignityOfLon = (planet, lon) => dignityOfSign(planet, signIndexOf(lon));
