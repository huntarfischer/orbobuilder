...
import { positions, wrap180 } from './ephem.js';
import { ASPECTS, midpoint } from './framing.js';
...
// for the same reason).
export const DEFAULT_TRANSIT_BODIES = ['Sun','Moon','Mercury','Venus','Mars','Jupiter','Saturn','Uranus','Neptune','Pluto','Node','Chiron','Lilith'];
...
export function natalTarget(natal) {
...
export function compositeTarget(natal, transitPosFn = positions) {
  return (jd) => {
...
export function compositeABTarget(natalA, natalB) {
  const out = {};
 ...
 */
export function scanTransitHits(opts) {
  const {
 ...
 */
export function nextExactHits(opts, jdFrom, count = 10, maxWindowDays = 730) {
