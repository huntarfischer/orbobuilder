// framing.browser.js — auto-generated browser-global build of framing.js (no ES modules; assigns window.__ORBO_FRAMING).
// Source of truth is framing.js — regenerate this file if framing.js changes, don't hand-edit.
...
const { norm360, wrap180, julianDay, jdToDate, positions, angles, findAscAnchor, BODY_ORDER } = window.__ORBO_EPH;
const { MARKS, separation, arcOf } = window.__ORBO_RING;
const { SIGNS, SIGN_GLYPHS, RULERS, dignityOfSign } = window.__ORBO_MATER;
...
const GLYPHS = { Sun:'☉', Moon:'☽', Mercury:'☿', Venus:'♀', Mars:'♂', Jupiter:'♃', Saturn:'♄', Uranus:'♅', Neptune:'♆', Pluto:'♇', Node:'☊', SNode:'☋', Chiron:'⚷', Lilith:'⚸', Asc:'AC', Mc:'MC', Fortune:'⊗', Spirit:'Spi', Eros:'Ero', Necessity:'Nec', Courage:'Cou', Victory:'Vic', Nemesis:'Nem' };
...
const LOT_SHORT = { Fortune:'For', Spirit:'Spi', Eros:'Ero', Necessity:'Nec', Courage:'Cou', Victory:'Vic', Nemesis:'Nem' };
function lots(asc, isDay, pos) {
  const L = {};
...
function midpoint(a, b) { return norm360(a + wrap180(b - a) / 2); }

// ---------- the refraction door (Phase 6 · P1) ----------
// THE ONE PLACE IN ORBO THAT REFRACTS — see framing.js for the full note. Generated build: edit the
// .js source of truth, never this file.
function refract(natalLon, momentLon) {
...
function houseOf(lon, ascLon) { return houseOfSign(signIndex(lon), signIndex(ascLon)); }

// ---------- natal ----------
function computeNatal(p) {
...
function frameFor(natal, jdLocalMidnightUT) {
...
function phaseOf(natalLon, transLon) { return norm360(transLon - natalLon) >= 180 ? 1 : 0; }
// the mod-180 coordinate: what is stored. Both ends of the permitted arc are axis and axis+180.
function axisOf(natalLon, transLon) { return norm360(midpoint(natalLon, transLon)) % 180; }
// the whole triple, as it rides on the genome: displayed point, axis, phase, permitted arc.
function axialOf(natalLon, transLon) {
...
const SYN_ORB_FACTOR = { 0: 1, 180: 1, 120: 0.84, 90: 0.84, 60: 0.67, 45: 0.34, 135: 0.34, 30: 0.34, 150: 0.34, 72: 0.34, 144: 0.34 };
function synOrb(angle, natalOrb = 6) { return (natalOrb / 2) * (SYN_ORB_FACTOR[angle] ?? 0.34); }

// ---------- arcs ----------
function arcFor(natalLon) { return { center: norm360(natalLon), start: norm360(natalLon - 90), end: norm360(natalLon + 90) }; }
function inArc(lon, arc) { return Math.abs(wrap180(lon - arc.center)) <= 90.0001; }
// overlap of two 180° arcs → array of [start,end] segments (0–2)
function arcOverlap(a1, a2) {
...
function overlapLen(a1, a2) { const d = Math.abs(wrap180(a1.center - a2.center)); return Math.max(0, 180 - d); }

// ---------- scans ----------
function scanFrames(natal, jdStart, days, cache) {
  const frames = [];
...
function frameEvents(natal, frames, jdStart) {
  const ev = [];
...
const FLIP_BODIES = BODIES.filter((b) => b !== 'SNode');
...
function synSpeed(bodySpeed) { return bodySpeed / 2; }
...
function flipEvents(natal, jdStart, jdEnd, posAt, opts = {}) {
...
const SYN_KINDS = ['flip', 'ingress', 'aspect'];
...
function synEvents(natal, jdStart, jdEnd, posAt, opts = {}) {
  const L = opts.loom;
...
function beadFamily(natalLonA, natalLonB) {
...
function beadMode(fam, phaseA, phaseB) {
...
function beadModeDays(fam, periodDays) {
  const d = fam.sep;
...
const LOOM_LAYERS = ['floor', 'contact', 'synchronic'];
const SIGN_BOUNDARIES = [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330];
...
function mod180(x) { const v = norm360(x) % 180; return v; }
...
function contactTargets(natal, opts = {}) {
...
function synchronicTargets(natal, opts = {}) {
...
function loomTargets(layer, natal, opts) {
...
// ---------- dispositor chain ----------
function dispositorChain(startBody, compPos) {
...
const ASPECTS = Object.freeze(ASPECT_ADMITS.map((angle) => {
...
function aspectBetween(a, b, orbMax) {
...
const dignityOf = (body, sign) => dignityOfSign(body, sign);
...
function scoreDay(natA, frameA, natB, frameB, profileKey, orbMax, frameA2, frameB2) {
...
function scoreMomentSolo(natal, pos, posN, profileKey, orbMax = 3) {
  const prof = PROFILES[profileKey];
  const comp = {}, compN = {};
...
window.__ORBO_FRAMING = { SIGNS, SIGN_GLYPHS, RULERS, GLYPHS, LOTS, LOT_SHORT, lots, BODIES, norm360, wrap180, julianDay, jdToDate, positions, angles, findAscAnchor, signIndex, degInSign, fmtLon, midpoint, refract, houseOf, computeNatal, frameFor, phaseOf, axisOf, axialOf, SYN_ORB_FACTOR, synOrb, arcFor, inArc, arcOverlap, overlapLen, scanFrames, frameEvents, FLIP_BODIES, synSpeed, flipEvents, SYN_KINDS, synEvents, synShape, beadFamily, beadMode, beadModeDays, LOOM_LAYERS, SIGN_BOUNDARIES, mod180, SYN_CONFIRM, floorTargets, contactTargets, synchronicTargets, loomTargets, dispositorChain, ASPECTS, aspectBetween, dignityOf, PROFILES, scoreDay, scoreMomentSolo };
})();
