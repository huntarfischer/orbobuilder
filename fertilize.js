...
import { BODIES, SIGNS, RULERS, signIndex, norm360, wrap180, synOrb } from './framing.js';
import { buildContact, buildSynchronic } from './loom.js';
...
export function fertKey(natal, doctrineKey) {
  const id = natal.seq
    ? 'g' + natal.seq
...
export function* fertilizeChunks(natal, spec) {
...
export function fertilize(natal, spec) {
...
export async function fertilizeAsync(natal, spec = {}) {
  const it = fertilizeChunks(natal, spec);
...
export function packWeave(built, meta = {}) {
...
export function unpackWeaveRow(p, i) {
...
export function readWeave(packed, q = {}) {
...
export function weaveStats(packed) {
