...
private enum AssemblyProof {
    ...
    private static func relationshipDirectionalDegree(
        ...
        if let text = optionalText(fields, header, directionalAliases),
           !text.isEmpty {
        ...
        for bound in eclipseFiles {
            ...
            _ = try streamCSV(url) { header, fields, rowNumber in
                ...
                guard let kind = MundaneTimespineEclipseKind(rawValue: kindText),
                      ...
                      bone.contains(phase),
                      let greatest = greatestValue.flatMap(JulianDay.init) ?? (greatestValue == nil ? nil : nil),
                      greatest == nil || bone.contains(greatest!),
                      let event = MundaneTimespineEclipseEvent(
                        kind: kind,
                        type: type,
