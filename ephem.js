...
export function wrap180(x) { x = norm360(x); return x > 180 ? x - 360 : x; }

// ---------- time ----------
export function julianDay(y, mo, d, h = 0, mi = 0, s = 0, utcOffset = 0) {
...
export function jdToDate(jd) { // returns JS Date (UTC)
...
export const ASTEROID_ORDER = ['Ceres', 'Pallas', 'Juno', 'Vesta'];
...
// ---------- positions of date ----------
export const BODY_ORDER = ['Sun','Moon','Mercury','Venus','Mars','Jupiter','Saturn','Uranus','Neptune','Pluto','Node','SNode','Chiron','Lilith'];
export function positions(jdUT) {
...
export function moonState(jdUT) {   // {lon, lat} deg of date, {r} km geocentric
  return moon3D(jdUT + deltaTdays(jdUT));
}
export function sunState(jdUT) {    // {lon} deg of date, {r} km geocentric
...
export function bodyLon(jdUT, name) {
  const jdTT = jdUT + deltaTdays(jdUT);
  const T = (jdTT - 2451545) / 36525;
...
export function angles(jdUT, lat, lon) { // lon east-positive
...
export function vertex(jdUT, lat, lon) {
  const ramc = norm360(gmst(jdUT) + lon);
...
// The other seven lots are NOT here: a lot needs no jd and no place, so it is not an ephemeris
// read but an expression of a decoded moment. They live in astrodna.js, with `extras` (2026-08-05).
export function partOfFortune(sunLon, moonLon, ascLon, isDay) {
...
export function findAscAnchor(jdStart, targetAsc, lat, lon) {
