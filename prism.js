...
// `refract` arrives here for P4 and for no other reason: the walk resolves to a DEGREE, and there is
// exactly one place in Orbo that turns a natal degree and a moment degree into one. The closed form
// `c + sigma - 180*pole` is algebraically identical and is deliberately NOT written, because a second
// expression for a refracted degree is a second refraction path however correct it is.
import { norm360, wrap180, arcFor, beadFamily, refract, signIndex, SIGNS, BODIES, LOTS, SYN_CONFIRM, ASPECTS, synOrb } from './framing.js';
import { houseOfSign, rulerOfHouse, FLIP_HOUSES } from './tympan.js';
import { MARKS, nearest, separation } from './ring.js';
// P5 · the ledger reuses the ONE scanner rather than writing a second. See §7's doc comment below for
// why an sASC-vs-occupant mark needs no change to loom.js at all.
import { scanTargets } from './loom.js';
...
import { gmst, obliquity } from './ephem.js';
...
export const RAMC_RATE = 360.98564736629;
export const SIDEREAL_DAY = 360 / RAMC_RATE;   // 0.99726957 mean solar days — one rotation
...
export function segmentsOf(natalLon, ascSignIdx) {
...
export function reachableOf(natalLon, ascSignIdx) {
...
export function familiesOf(natalA, natalB, orb = 3) {
...
export function itineraryOf(natalAscLon, ascSignIdx) {
...
export function risingRamc(lon, lat, eps) {
...
export function ramcJdNear(ramcTarget, jdNear, lonEast) {
...
export function templateOf(natalAscLon, ascSignIdx, lat, opts = {}) {
...
export function walkOf(natalAscLon, horizonLon) {
...
export function stopAtWalk(template, sigma) {
...
export function dialOf(template, natalAscLon, horizonLon, opts = {}) {
...
// The itinerary gives seven stops; the template gives each one a real RAMC and a real duration. What
// P5 adds is the READING: which row a given moment falls in, and which marks land inside which row.
//
// stopAtWalk answers "which row" from sigma, the walk. This answers the SAME question from the local
// RAMC directly, with no walk, no horizon and no ephemeris beyond gmst — because a stop's `ramc` is
// already an absolute rotation angle (stops[0].ramc IS the anchor's ramc, sigma=0) and every stop's
// `ramcSpan` is the forward gap to the next, so the same cumulative sum templateOf used to CUT the day
// answers "what is open right now" from a bare rotation reading. This is not a second definition of a
// row: it is the one definition, read through a different door, and the two must always agree (the P5
// self-test below cross-checks them on a live fixture rather than assuming it).
export function stopAtRamc(template, ramcTarget) {
  if (!template || template.circumpolar || !template.stops.length) return null;
...
// The read-time inverse of ramcJdNear's own internal `local()`, exported so a caller can bucket ANY
// jd (a scanner root, a live cursor) into a row without touching the horizon or the ephemeris beyond
// gmst. Latitude never enters — RAMC is a longitude-and-time fact only, same as everywhere else here.
export function localRamc(jd, lonEast) { return norm360(gmst(jd) + lonEast); }

// ---------- 8 · P5 · THE MARKS · sASC-vs-occupant, on the one scanner ----------
// §4's open engineering question ("the one-scanner tension"), resolved the way this codebase resolves
// them: build it on the scanner FIRST, never write a second one. A mark "to occupant P" is nothing but
// an ordinary mode:'sep' target between two occupants where one of them happens to be the sASC — and
// the sASC is already an occupant under loom's own algebra (lonAt = nat==null ? sky : refract(nat,
// sky)), with `nat` the natal ASCENDANT and `sky` the horizon degree at that instant. So this needs NO
// change to loom.js: the caller hands scanTargets a probe that answers `Asc` (the horizon) alongside
// the ordinary bodies — e.g. spine.ascProbe — and the existing scanner does the rest, under the same
// residency guard and the same wrap guard every other layer already relies on.
//
// ORBS HALVE on this layer (framing.synOrb), and the admitted angles are the same five majors used
// everywhere else in the synchronic weave — always the Ring's word for the mark (framing.ASPECTS),
// never a second table of names.
//
// THE MOON IS EXCLUDED BY DEFAULT. At 6.6°/day synchronic she is not parked within a day (§1.3), so a
// sASC-vs-Moon scan would emit on the order of a dozen marks and drown the other six bodies' handful.
// She is luna.js's business, exactly as she already is on every other layer — a fourth kind read
// alongside the row, never a row occupant herself.
const LEDGER_BODIES_DEFAULT = BODIES.filter((b) => b !== 'Moon' && b !== 'SNode');

export function sascTargets(natal, opts = {}) {
...
// The day's marks: every sASC-vs-occupant root inside [jdStart, jdEnd), decorated just enough to
// place it (the Ring's word, the body, the row it falls in). `probe(jd)` must answer `Asc` alongside
// the ordinary bodies; `lonEast` buckets each root into a row through stopAtRamc, which is a pure
// rotation lookup and touches nothing else. A generous default step (STEP_FOR carries no Ascendant
// entry, per §4): the sASC moves 180°/day and marks come as close as 6° apart, so 0.01 days is well
// under both the "don't skip a root" and loom's own comment thresholds.
export function ledgerMarks(natal, template, probe, jdStart, jdEnd, opts = {}) {
...
// The whole reading: the itinerary's seven stops, each carrying the marks whose root falls inside it.
// `[jdStart, jdEnd)` should span one rotation (a caller reading "today" passes the day's own anchor
// and anchor + SIDEREAL_DAY), which tiles the stops exactly with no double-count at the seam. NEVER
// stored: this is computed at read time from the template, per the pre-ruling upheld through every
// prior pass — a ledger row is a reading, not an event on the spine.
export function ledgerOf(natal, template, probe, jdStart, jdEnd, opts = {}) {
...
// "You cannot choose where Saturn is. You choose when you walk in the door, and that choice sets the
// sASC and every relation it forms." Natal electional (electional.js) scores WHERE THE BODIES ARE;
// this scores WHAT YOUR ARRIVAL DOES TO THE REFRACTED CHART, which is entirely yours.
//
// THE SEARCH SPACE IS SEVEN WINDOWS, NOT A GRID (§3): the itinerary is fixed at engrave, so the stops
// are already known, and a minute-grid sweep of the whole day is the wrong shape here (it also risks
// skipping a root exactly as STEP_FOR does at its default). ♏ Timing already owns electional — THIS IS
// NOT A SECOND ENGINE. It is a much smaller thing, scoped to the three criteria only the sASC has: a
// moving Ascendant's own arc-confined stop, its halved-orb marks (P5's ledger, reused verbatim — one
// path to a mark, never two), and where in the stop an arrival lands.
//
// Reuses framing.PROFILES (sigs/holders/houses) so "what you're electing for" is the SAME vocabulary
// the natal engine already uses; the app hands both readings the identical profile choice.
const HARMONY = { 0: 0, 60: 1, 90: -1, 120: 1, 180: -1 };   // the same numbers as framing.Q / electional.HARMONY

function stopSigWeight(body, prof) { return prof.sigs.includes(body) ? 1.6 : prof.holders.includes(body) ? 1.15 : 0; }

// Criterion 1 (the stop itself) and criterion 2 (its marks). Deliberately blind to `st.afterFlip`: the
// flip is an ordinary boundary crossing of the same class as the other six (P4), so scoring it as an
// exception here would reintroduce in the scorer exactly what P4 deleted from the detector.
function scoreStop(row, prof) {
  ...
  // ALWAYS QUALIFIED (\u00a711.4): never "lord Mercury" \u2014 the sky's stays "rising lord", this one is
  // always said in full so the two are never mistaken for each other on one pane.
...
// The query itself: rank the window's stops by scoreStop, then refine INSIDE each stop rather than
// re-scanning \u2014 the only candidate arrivals considered are the stop's own midpoint (the structural
// default) and every mark P5's ledger already found inside it (a mark IS a candidate arrival, never a
// re-scanned one). `probe`/`jdStart`/`jdEnd`/`lonEast` are exactly P5's own ledger inputs.
//
// A PLACE IS PART OF THE QUESTION (\u00a78): the sASC is an angle, so an election computed at one city is
// not valid at another. Refuses (null) rather than answering for the wrong place, exactly as a
// placeless or circumpolar chart gets no ledger.
export function queryOf(natal, template, probe, jdStart, jdEnd, profile, opts = {}) {
  ...
  for (const row of ledger.rows) {
    ...
    for (const inst of insts) {
      ...
      if (!at || at.index !== row.index) continue;   // criterion 1: never leaves the stop's own window
      // criterion 3: where in the stop. Penalize the first/last ~15% \u2014 a minute before a Synchronic
      // Ascendant Ruler handoff and a minute after are different elections.
...
export function prismKey(natal, opts = {}, doctrineKey) {
...
export function build(natal, opts = {}) {
...
export function buildPair(natalA, natalB, opts = {}) {
...
(function selfTest() {
  ...
  // ── P5 · the ledger ────────────────────────────────────────────────────────────────────────
  // THE BAR, proved WITHOUT a probe: stopAtRamc (a bare rotation lookup) and stopAtWalk (a bare walk
  // lookup) must agree on every instant of the day, because they are two doors onto the SAME row. Both
  // sides are driven from risingRamc alone (already verified elsewhere to 1e-13° against the live
  // ephemeris), so this needs no ephemeris access of its own and no fake sky. The physically-live
  // version of this same check (against E.angles) lives in tests/prism.test.html, which is where the
  // acceptance suite's real proof is recorded; this is the structural half prism.js can prove alone.
  ...
  // A wiring smoke test for the scanner path itself (sascTargets \u2192 scanTargets \u2192 ledgerMarks): bodies
  // parked at their natal degree (sky == natal, so refract(nat, nat) is a constant) and a horizon that
  // advances roughly through the day \u2014 not physically exact (this file has no ephem.angles to check
  // against), so this proves the PLUMBING (every mark lands in a valid row, none are dropped or
  // duplicated) rather than the physical cross-check, which belongs to the live-ephemeris suite.
  ...
  // Criterion 1: every candidate's stop is the template's OWN stop object, never re-derived.
  ...
  // Criterion 2: a candidate's marks are exactly the ledger's own row.marks — same array, not a
  // re-scanned copy — the max-delta-0.00 pattern applied as object identity rather than a time delta.
  ...
  // The flip is scored as an ordinary boundary crossing: scoreStop must not read st.afterFlip at all.
  ...
  // No unqualified lord: every holder-driven driver names the Synchronic Ascendant Ruler in full.
  ...
  // A place is part of the question: no lonEast, no query. And a circumpolar template gets none either.
  ...
  // ── P7 · synastry ──────────────────────────────────────────────────────────────────────────
  // THE HEADLINE (§1): sASC_A − sASC_B is an ANGLE separation and does NOT cancel over place,
  // unlike every same-body separation, which cancels exactly regardless of place. AT A SHARED
  // PLACE the two Ascendants form a FAMILY under the exact same law as any shared body (the
  // horizon is the sky term, and it is shared, so it cancels): the raw separation takes one of
  // exactly two values, {\u03b4/2, 180\u2212\u03b4/2} with \u03b4 = |nASC_A \u2212 nASC_B|, alternating at either
  // native's own flip \u2014 never a single constant, which is the same-body mode law applied here.
  ...
  if (worstSamePlace > 1e-9) throw new Error(who + 'self-test: same place must hold the sASC separation to the pair\u2019s own {\u03b4/2,180\u2212\u03b4/2} family, drifted ' + worstSamePlace);
  // Different places: two horizons NOT tied together by a shared instant — decorrelated on
  // purpose, standing in for what a second, unrelated place's ascension contributes. The family
  // is exactly two points on a continuum, so a genuinely different relationship must miss it for
  // most samples. The live-ephemeris version of this same proof (two REAL places) lives in
  // tests/prism.test.html, which is where the acceptance suite's authoritative measurement is.
  ...
  // Same-body separations cancel EXACTLY regardless of place (P2's bar, re-asserted on the pair
  // path), and A\u00d7B must equal B\u00d7A \u2014 a family is a fact of the two charts, not an order.
