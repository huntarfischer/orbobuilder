#!/usr/bin/env python3
"""Manufacture Orbo's numbered Jupiter Reign table.

Numbering law:
  Z0 = canonical Pluto first direct Aries ingress (3532 BCE)
  J0 = first qualifying Jupiter direct Pisces->Aries ingress strictly after Z0

Each Reign owns [first_aries_ingress, next_reign_first_aries_ingress).
Later retrograde recrossings of 0 Aries belong to the same Reign.
...
# Wide enough to bind the final Reign that begins before the end of Z23.
SCAN_END = Z_BOUNDS["Z23"][1] + 20 * 365.25
SCAN_STEP_DAYS = 5.0
CLUSTER_GAP_DAYS = 4 * 365.25
...
class SwissC:
    ...
    def state(self, jd: float) -> tuple[float, float]:
        ...
        returned = int(self.lib.swe_calc_ut(jd, JUPITER, FLAGS, xx, serr))
        if returned < 0:
...
def transition_shadow(swiss: SwissC, first: float, final: float) -> tuple[float | None, str | None, float | None]:
    ...
    while travelled <= max_back:
        ...
        lo -= 2.0
        travelled += 2.0
    raise RuntimeError("Could not solve Jovian Reign pre-shadow start")
...
def manufacture(swiss: SwissC) -> list[dict]:
    ...
    if not rows or rows[0]["shell_id"] != "J0":
        raise RuntimeError("J0 was not manufactured")
    return rows
...
def audit(rows: list[dict]) -> dict:
    checks = {
        "row_count": len(rows),
        "j0_after_z0": rows[0]["first_aries_ingress_jd_ut"] > Z0_JD,
...
def main() -> int:
    ...
    try:
        ...
        if report["status"] != "PASS":
            raise RuntimeError(f"Jovian Reign audit failed: {report}")
