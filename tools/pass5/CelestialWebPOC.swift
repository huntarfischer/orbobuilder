...
struct CelestialWeb {
    ...
    func exactCrossings(body: String, degree: Int) throws -> [CrossingHit] {
        ...
        for motion in ["direct", "retrograde"] {
            for id in degreeIndex[motion] as? [Int] ?? [] { hits.append(CrossingHit(motion: motion, boundaryID: id)) }
        }
        return hits.sorted { $0.boundaryID < $1.boundaryID }
    }

    func cellSegments(body: String, degree: Int) throws -> [CellHit] {
        ...
        var hits: [CellHit] = []
        for motion in ["direct", "retrograde"] {
            for id in degreeIndex[motion] as? [Int] ?? [] { hits.append(CellHit(motion: motion, segmentID: id)) }
        }
    ...
    func specimenStartJD() throws -> Double {
        guard let start = bounds["start"] as? [String: Any],
              let address = start["resolvedAddress"] as? [String: Any],
              let jd = address["julianDay"] as? Double else {
            throw POCError.malformed("Missing specimen start address.")
        }
        return jd
    }
}
...
func run() throws {
    ...
    let crossings = try web.exactCrossings(body: body, degree: degree)
    ...
    let cellHits = try web.cellSegments(body: body, degree: degree)
    ...
    for hit in cellHits {
        ...
        let otherBodies = (web.tracts.keys.filter { $0 != body }).sorted()
    ...
    if let firstCrossing = crossings.first {
        ...
        for name in refs.keys.sorted() {
            ...
            let cell = try int(segment, "degreeCell")
            let motion = try string(segment, "motion")
            print("      \(name): tract \(ref), \(zodiac(cell)), \(motion)")
        }
    }
