...
enum ForgeError: Error, CustomStringConvertible {
    case usage(String), dl(String), swiss(String), io(String)
    var description: String {
        switch self { case .usage(let s), .dl(let s), .swiss(let s), .io(let s): return s }
    }
}
...
struct Event { let family:String, bodyA:String, bodyB:String, aspect:String, ring:Int, orientation:String, aLon:Double, bLon:Double, jd:Double, offset:Int64, residual:Double }
...
func relative(_ pair:Pair,_ jd:Double,_ target:Double,_ sw:Swiss)throws->(Double,State,State) {
    let a=try sw.state(pair.a,jd), b=try sw.state(pair.b,jd); let raw=norm(a.longitude-b.longitude); let u=raw+360*round((target-raw)/360); return (u-target,a,b)
}
func refine(_ pair:Pair,_ target:Double,_ lo0:Double,_ hi0:Double,_ sw:Swiss)throws->(Double,State,State) {
    var lo=lo0, hi=hi0; var gl=try relative(pair,lo,target,sw).0; var gh=try relative(pair,hi,target,sw).0
    if abs(gl)<1e-13 { let x=try relative(pair,lo,target,sw); return (lo,x.1,x.2) }
    if abs(gh)<1e-13 { let x=try relative(pair,hi,target,sw); return (hi,x.1,x.2) }
    var x=lo+(hi-lo)*abs(gl)/(abs(gl)+abs(gh))
    var last:(Double,State,State)?
    for _ in 0..<18 {
        let q=try relative(pair,x,target,sw); last=(q.0,q.1,q.2)
        if abs(q.0)<1e-12 || (hi-lo)*86400 < 0.0005 { return (x,q.1,q.2) }
        if gl*q.0<=0 { hi=x; gh=q.0 } else { lo=x; gl=q.0 }
        let deriv=q.1.speed-q.2.speed
        var candidate = abs(deriv)>1e-12 ? x-q.0/deriv : (lo+hi)/2
        if !(candidate>lo && candidate<hi) || !candidate.isFinite { candidate=(lo+hi)/2 }
        x=candidate
    }
    let mid=(lo+hi)/2, q=try relative(pair,mid,target,sw); return (mid,q.1,q.2)
}

func args()throws->(String,String,String,Double,Double,Double) {
...
func line(_ e:Event)->String { [e.bodyA,e.bodyB,e.aspect,String(e.ring),e.orientation,String(format:"%.9f",e.aLon),String(format:"%.9f",e.bLon),String(format:"%.12f",e.jd),String(e.offset),String(format:"%.6f",e.residual)].joined(separator:",")+"\n" }

func main()throws {
    let (lib,ephe,outPath,start,end,origin)=try args(); let sw=try Swiss(lib,ephe); guard sw.version=="2.10.03" else { throw ForgeError.swiss("unexpected Swiss \(sw.version)") }
    let out=URL(fileURLWithPath:outPath); try FileManager.default.createDirectory(at:out,withIntermediateDirectories:true)
    let major=out.appendingPathComponent("exact-major-mundane-transits.csv"), minor=out.appendingPathComponent("exact-minor-mundane-transits.csv")
    FileManager.default.createFile(atPath:major.path,contents:nil); FileManager.default.createFile(atPath:minor.path,contents:nil)
    let mh=try FileHandle(forWritingTo:major), nh=try FileHandle(forWritingTo:minor); defer{try? mh.close();try? nh.close()}
    let header="bodyA,bodyB,aspect,ringDegrees,orientation,bodyACelestialTimeDegrees,bodyBCelestialTimeDegrees,civicTimeJulianDayUT,civicTimeOffsetSeconds,exactAspectResidualArcSeconds\n".data(using:.utf8)!; try mh.write(contentsOf:header); try nh.write(contentsOf:header)
    ...
    let initial=try states(start); var progress:[Pair:Progress]=[:]
    for p in pairs { let raw=norm(initial[p.a]!.longitude-initial[p.b]!.longitude); progress[p]=Progress(raw:raw,unwrapped:raw) }
    var counts=["major":0,"minor":0], skipped=0; var lastKey:[String:(String,Int64)]=[:]
    var previousJD=start, jd=start+0.1
    while previousJD<end {
        let currentJD=min(jd,end), current=try states(currentJD)
        for p in pairs {
            var pr=progress[p]!; let raw=norm(current[p.a]!.longitude-current[p.b]!.longitude); let unwrapped=pr.unwrapped+signedDelta(pr.raw,raw)
            for t in targets {
                for absolute in crossings(pr.unwrapped,unwrapped,t.oriented) {
                    let q=try refine(p,absolute,previousJD,currentJD,sw); if q.0 < start || q.0 >= end { continue }
                    let offset=Int64(((q.0-origin)*86400).rounded()); if offset<0 { continue }
                    let residual=circError(norm(q.1.longitude-q.2.longitude),norm(absolute))*3600
                    let a9=String(format:"%.9f",q.1.longitude), b9=String(format:"%.9f",q.2.longitude)
                    let celestial="\(p.a.name)|\(p.b.name)|\(t.ring)|\(t.orientation)|\(a9)|\(b9)"
                    let dupKey="\(celestial)|\(offset)"
                    if let z=lastKey[t.family], z.0==dupKey && abs(q.0-Double(z.1))<0 { skipped += 1; continue }
                    // Exact duplicate-source law is enforced globally in the post-manufacture audit; boundary adjacency is de-duped here by stored identity.
                    let e=Event(family:t.family,bodyA:p.a.name,bodyB:p.b.name,aspect:t.aspect,ring:t.ring,orientation:t.orientation,aLon:q.1.longitude,bLon:q.2.longitude,jd:q.0,offset:offset,residual:residual)
                    let data=line(e).data(using:.utf8)!; if t.family=="major" { try mh.write(contentsOf:data) } else { try nh.write(contentsOf:data) }; counts[t.family,default:0]+=1; lastKey[t.family]=(dupKey,Int64(q.0.bitPattern))
                }
            }
    ...
    print("PASS relationship forge Swiss=\(sw.version) start=\(start) end=\(end) major=\(counts[\"major\"]!) minor=\(counts[\"minor\"]!) skipped=\(skipped)")
}
