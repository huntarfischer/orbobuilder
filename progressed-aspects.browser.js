// progressed-aspects.browser.js — auto-generated browser-global build of progressed-aspects.js
// (no ES modules; assigns window.__ORBO_PROGRESSED_ASPECTS). Source of truth is
// progressed-aspects.js — regenerate this file if it changes, don't hand-edit.
// Load order: after progressions.browser.js.
(function boot(){
if(!window.__ORBO_PROGRESSIONS){return void setTimeout(boot,0);}
const { TROPICAL_YEAR_DAYS, realJdFromAge } = window.__ORBO_PROGRESSIONS;
...
const DEFAULT_BODIES = Object.freeze(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune']);
...
function build(natal, sample, opts = {}) {
...
function hitsNear(table, jd, windowDays) {
  if (!table) return [];
...
function tableKey(natal, bodies) {
  return natal.jd + '|' + (bodies || DEFAULT_BODIES).slice().sort().join(',');
}

window.__ORBO_PROGRESSED_ASPECTS = { ASPECTS: ASPECTS, DEFAULT_BODIES: DEFAULT_BODIES, build: build, hitsNear: hitsNear, tableKey: tableKey };
})();
