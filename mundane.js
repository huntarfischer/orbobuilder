...
import { scanTargets, decorate, buildFloor, retroPeriods } from './loom.js';
import { floorTargets, norm360, wrap180, signIndex, SIGNS, RULERS, BODIES } from './framing.js';
// The Ring is the floor and imports nothing, so depending on it adds no cycle. Angles only:
// the eclipse constants, the sign names and the codec's dictionaries are MEANING and stay here.
import { MARKS } from './ring.js';

export const EMBRYO_SPAN = { y0: 1700, y1: 2100 };
...
export function angSep(l1, b1, l2, b2) {
...
export function eclipseAt(jd, kind, dep) {
...
export function eclipsesIn(spec) {
...
export function applyCanon(eclipses, canon, opts = {}) {
...
export const KINDS = ['ingress', 'station', 'retrograde', 'syzygy', 'eclipse', 'aspect'];
...
export function packEmbryo(rows, span) {
...
export function readEmbryo(packed, q = {}) {
...
export const SLICE = 1461;   // four years, about a Mars synodic handful of stations per pass

export function* mundaneChunks(spec) {
...
export function buildMundane(spec) {
