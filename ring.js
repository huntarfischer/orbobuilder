...
export const MARKS = Object.freeze([0, 30, 45, 60, 72, 90, 120, 135, 144, 150, 180]);
export const DEGREES = 360;
export const STATES = 720;
export const TIE = 'lower';
...
export const ARCSEC = 60 * 60;
export const ARCSECONDS = DEGREES * ARCSEC;   // 1,296,000 · one circle, exactly
export const FINE_STATES = ARCSECONDS * 2;    // 2,592,000 · direct half, retrograde half
...
export const degreeOf = (state) => deg(ckState(state, 'degreeOf'));
export const isRetro = (state) => ckState(state, 'isRetro') >= DEGREES;
export const directState = (d) => Math.floor(norm360(ckFinite(d, 'directState'))) % DEGREES;
export const retroState = (d) => directState(d) + DEGREES;
export const statesFor = (d) => { const s = directState(d); return [s, s + DEGREES]; };
// An occupant's address. lon is real-valued; the state is its whole degree plus its motion.
export const stateOf = (lon, retro) => (Math.floor(norm360(ckFinite(lon, 'stateOf'))) % DEGREES) + (ckBool(retro, 'stateOf') ? DEGREES : 0);
...
export const arcsecOf = (fine) => asec(ckFine(fine, 'arcsecOf'));
export const fineIsRetro = (fine) => ckFine(fine, 'fineIsRetro') >= ARCSECONDS;
export const fineStateOf = (lon, retro) =>
  (Math.floor(norm360(ckFinite(lon, 'fineStateOf')) * ARCSEC) % ARCSECONDS) + (ckBool(retro, 'fineStateOf') ? ARCSECONDS : 0);
export const fineStatesFor = (lon) => { const f = fineStateOf(ckFinite(lon, 'fineStatesFor'), false); return [f, f + ARCSECONDS]; };
...
export const stateOfFine = (fine) => Math.floor(asec(ckFine(fine, 'stateOfFine')) / ARCSEC) + (fine >= ARCSECONDS ? DEGREES : 0);
...
export function targetDegree(state, angle, sign) {
...
export function targetStates(state, angle, sign) {
...
export function relation(stateA, stateB) {
...
export function related(stateA, stateB) {
...
export const separation = (lonA, lonB) => norm360(ckFinite(lonB, 'separation') - ckFinite(lonA, 'separation'));
export const arcOf = (sep) => { const s = norm360(ckFinite(sep, 'arcOf')); return s <= 180 ? s : 360 - s; };
...
export function exact(sep) { const n = nearest(sep); return n.residual === 0 ? n.angle : null; }
...
export function supplementOf(angle) {
