...
export const CLASSICAL = ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn'];
...
export function sectWeight(body, isDay) {
...
// ---------------------------------------------------------------------------
export function essentialDignity(body, lon, isDay) {
...
// ---------------------------------------------------------------------------
export function combustState(lon, sunLon) {
...
export function condition(body, ctx) {
...
export function gradeAspect(bodyA, bodyB, lonA, lonB, orbMax, ctx, applying) {
...
export function nextMoonAspect(natal, jd, posAt, opts = {}) {
...
export function moonModule(natal, ctx, jd, posAt, opts = {}) {
...
// ---------------------------------------------------------------------------
export function lotBlock(ctx, profileKey) {
...
export function fixedStarBlock(ctx, sigs) {
  const drivers = []; let score = 0;
...
// ---------------------------------------------------------------------------
export function fieldBlock(natal, ctx, jd, posAt) {
  const drivers = []; let score = 0; let veto = null;
...
export function arcBound(natal, sig, houses, cAsc) {
...
// ---------------------------------------------------------------------------
export function bodySpeed(jd, body, posAt) {
  const h = 0.02;
  return wrap180(posAt(jd + h)[body] - posAt(jd - h)[body]) / (2 * h);
}
export function speedModel(natal, jd, posAt) {
...
export function applyingState(ctx, b1, b2, mode = 'frame') {
...
export function gradeSynastry(natal1, natal2, jd, posAt, opts = {}) {
...
// ---------------------------------------------------------------------------
export const WEIGHTS = { moon: 0.30, actor: 0.20, topic: 0.20, angles: 0.12, aspects: 0.10, field: 0.05, lots: 0.02, fixstar: 0.01 };
...
// weighting. Nodes kept and Node/SNode listed SEPARATELY (the scorers drop SNode
// as a mirror; here both are wanted). Chiron/Lilith excluded to match the body
// set. The exact-time scan + travel live in the app. posAt: jd->positions.
// ---------------------------------------------------------------------------
...
export function crossAspects(natA, natB, jd, posAt, opts = {}) {
...
// ---------------------------------------------------------------------------
export function scoreMomentSynastry(natA, natB, jd, posAt, profile, opts = {}) {
  const orb = opts.orb ?? 3;
  const mom = posAt(jd);
  const compA = {}, compB = {};
...
export function scoreDayV2(natA, natB, jdLocalMidnight, posAt, profile, opts = {}) {
...
export function scoreDaySolo(natal, jdLocalMidnight, posAt, profiles, opts = {}) {
...
export function scoreMomentV2(natal, jd, posAt, profileKey, opts = {}) {
